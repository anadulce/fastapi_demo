...
from src.routers.schema import MovieInSchema, MovieOutSchema, MoviePartialUpdateSchema, MovieBDSchema, PageMovieSchema, Message
from src.crud import create, get_all, get_one, update, delete, get_offset

router = APIRouter(prefix='/movie', tags=['movies'])


@router.post('/', status_code=HTTPStatus.CREATED, response_model=MovieOutSchema)
def create_movie(movie: MovieInSchema, session: Session = Depends(get_session)):
...
def read_movies_by_page(page: int = 1, limit: int = 100, session: Session = Depends(get_session)):
...
def update_movie(id:int, movie_to_update: MovieInSchema, session: Session = Depends(get_session)):
...
@router.patch('/{id}/', response_model=MovieOutSchema)
def partial_update_movie(id:int, movie_to_update: MoviePartialUpdateSchema, session: Session = Depends(get_session)):
    movie_to_update
    movie, message =  update(session, Movie, id, movie_to_update)
    if movie:
        return movie
